...
namespace NalogAdministrator.Classes.ImpData.Methods
{
    public class ImportDataSubjectIndex
    {
        ...
        public void AddRecords(string sheet, string tag)
        {
            ...
            var.InternalTable.TableName = impData.ClearStrExcelTable(sheet);
                            ...
                            for (int j = 0; j < var.Years.Count; j++)
                            {
                                    ...
                                    var.IdSubjNow > 0)
                                {
                                    ...
                                    if (status == 0)
                                    {
                                        ...
                                        subjectIndexRow[var.InternalTable.TableName] = impData.returnNullDecimal(var.InternalTable.Rows[var.IndexRecord][var.Years[j][0]], 2);
