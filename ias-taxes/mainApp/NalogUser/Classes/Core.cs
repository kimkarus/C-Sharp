...
namespace NalogUser.Classes
{
    public class Core
    {
        ...
        public Core(FormApp FrmAppl=null)
        {
            ...
            b = new Data.Base(null, null, this);
        ...
        private void OnTakeScreenShot(int width = 0, int height = 0, int x = 0, int y = 0, string name="")
        {
            ...
            string folderPathScreenshots = this.cfg.AppPath + "\\screenshots";
            bool isFolderScreenshotsExists = Directory.Exists(folderPathScreenshots);

            if (!isFolderScreenshotsExists)
            {
                Directory.CreateDirectory(folderPathScreenshots);
            }
            if (name.Length > 0)
            {
                bmp.Save(folderPathScreenshots + "\\" + name + ".bmp");
            }
            else
            {
                bmp.Save(folderPathScreenshots + "\\screenshot.bmp");
            }
        ...
        public class _Config
        {
            ...
            private void setConfig()
            {
                ...
                //
                elem = (XmlElement)node.SelectSingleNode(string.Format("//add[@key='{0}']", "Username"));
                NalogUser.App.Default.Username = elem.GetAttribute("value");
                //
            ...
            private string[] loadConfigIniDocument()
            {
                ...
                {
                    ini = File.ReadAllLines(configIniPath);
