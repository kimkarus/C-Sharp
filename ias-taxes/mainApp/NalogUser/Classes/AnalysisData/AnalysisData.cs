                    ...
                    case 0:
                        {
                            ...
                            try
                            {
                                ...
                                if (done && var.AddZeroToReport)
                                {
                                    ...
                                    string[] columns = { "lawrence_accum_share_ti_in_district", "lawrence_accum_share_busyed_in_district", "lawrence_accum_share_ti_ideal_in_district","lawrence_accum_share_busyed_ideal_in_district", "year" };
                    ...
                    case 1:
                        {
                            ...
                            {
                                //SQL
                                cr.B.Parametr.IdSubject.sqlP = new System.Data.SqlClient.SqlParameter(cr.B.Parametr.IdSubject.name, var.IdSubject);
                                cr.B.BaseSql.ArrayParamsIdsSubjects.Add(cr.B.Parametr.IdSubject.sqlP);
                                cr.B.BaseSql.DbCom.Parameters.Add(cr.B.Parametr.IdSubject.sqlP);
                            }
                            if (cr.B.IsOledb)
                            {
                                //OLEDB
        ...
        public DataTable requestMapMoreInfoSubject(int id_subject = 0, short year = 0, int id_tax=0)
        {
            ...
            if (id_subject != 0 && year != 0)
            {
                ...
                if (cr.B.IsSql)
                {
                    ...
                    if (var.IdDistrict > 0) { cr.B.BaseSql.DbCom.Parameters.Add(cr.B.Parametr.IdDistrict.sqlP); cr.B.BaseSql.DbCom.Parameters[cr.B.Parametr.IdDistrict.name].Value = var.IdDistrict; }
                    //
                    cr.B.Parametr.IdSubject.sqlP = new System.Data.SqlClient.SqlParameter(cr.B.Parametr.IdSubject.name, var.IdSubject);
                    cr.B.BaseSql.ArrayParamsIdsSubjects.Add(cr.B.Parametr.IdSubject.sqlP);
                    ...
                    //
                    if (var.IdSubject > 0) { cr.B.BaseSql.DbCom.Parameters.Add(cr.B.Parametr.IdSubject.sqlP); cr.B.BaseSql.DbCom.Parameters[cr.B.Parametr.IdSubject.name].Value = var.IdSubject; }
                    if (var.IdTax > 0) { cr.B.BaseSql.DbCom.Parameters.Add(cr.B.Parametr.IdTax.sqlP); cr.B.BaseSql.DbCom.Parameters[cr.B.Parametr.IdTax.name].Value = var.IdTax; }
                    if (var.IdEea > 0) { cr.B.BaseSql.DbCom.Parameters.Add(cr.B.Parametr.IdEea.sqlP); cr.B.BaseSql.DbCom.Parameters[cr.B.Parametr.IdEea.name].Value = var.IdEea; }
                    if (var.YearAfter > 0) { cr.B.BaseSql.DbCom.Parameters.Add(cr.B.Parametr.YearAfter.sqlP); cr.B.BaseSql.DbCom.Parameters[cr.B.Parametr.YearAfter.name].Value = var.YearAfter; }
                    if (var.YearBefore > 0) { cr.B.BaseSql.DbCom.Parameters.Add(cr.B.Parametr.YearBefore.sqlP); cr.B.BaseSql.DbCom.Parameters[cr.B.Parametr.YearBefore.name].Value = var.YearBefore; }
                    //
                ...
                if (cr.B.IsOledb)
                {
                    ...
                    if (var.IdDistrict > 0) { cr.B.BaseSql.DbCom.Parameters.Add(cr.B.Parametr.IdDistrict.oledbP); cr.B.BaseSql.DbCom.Parameters[cr.B.Parametr.IdDistrict.name].Value = var.IdDistrict; }
                    //
                    cr.B.Parametr.IdSubject.oledbP = new System.Data.OleDb.OleDbParameter(cr.B.Parametr.IdSubject.name, var.IdSubject);
                    cr.B.BaseOleDb.ArrayParamsIdsSubjects.Add(cr.B.BaseOleDb.ParamIdSubject);
                    //cr.B.BaseSql.DbCom.Parameters.Add(cr.B.BaseSql.ParamIdSubject);
                    //
                    //cr.B.BaseSql.DbCom.Parameters.Clear();
