...
namespace NalogUser.Forms
{
    ...
    public partial class FormAnalysisDataOnMap : Form
    {
        ...
        private decimal[,] matches;
        private bool dataIsPercent = false;
        ...
        public FormAnalysisDataOnMap(Classes.Core Cr, bool DataFromChoiceForm)
        {
            ...
            string formText = "";
            string textYear = "";
            textYear = cr.AnalysData.Var.YearAfter.ToString() + " - " + cr.AnalysData.Var.YearBefore.ToString();
            if (cr.AnalysData.Var.YearAfter == cr.AnalysData.Var.YearBefore) textYear = cr.AnalysData.Var.YearBefore.ToString();
            this.Text += " " + cr.AnalysData.Var.NameTextReport + " " + textYear;
            //
            hideMorePanel(true);
            //clearMapElements();
        }
        ...
        private void selectDataSubjects()
        {
            ...
            defineTypeDataFormat(cr.AnalysData.Var.ActiveColSourceTable);
            ...
            if (dataIsPercent)
            {
                matches = new decimal[,] { { 0, 15, 20, 25, 40, 60, 100 }, { 6, 6, 6, 6, 6, 6, 6} };
            }
            else
            {
                matches = new decimal[,] { { 0, 15, 20, 25, 40, 60, 100 }, { 6, 6, 6, 6, 6, 6, 6 } };
            ...
            for (int i = 0; i < matches.Length/2; i++)
            {
                ...
                {
                    intervals = cr.AnalysData.returnNullInt(matches[1, i]);
                }

            }

            countOfColors = intervals; //Количесвто цветов, используемы для заполнения карты
            if (countOfColors <= 0) { MessageBox.Show("Выборка данных слишком мала. Попробуйте изменить год"); return; }
            ...
            { 
                stepSum -= step;
                if (stepSum < 0) stepSum = 0;
                listColors.Add(System.Drawing.Color.FromArgb(255, stepSum, stepSum)); 
            }
            //Добавляем диапазоны значений
            listRangesSourveValue.Clear();
            decimal range = (maxSourceValue + Math.Abs(minSourceValue)) / countOfColors;
            decimal rangeSum = minSourceValue;
            if (dataIsPercent)
            {
                for (int i = 0; i < matches.Length / 2; i++) { if (matches[0, i] > 0) { listRangesSourveValue.Add((decimal)matches[0, i] / 100); } else { listRangesSourveValue.Add((decimal)matches[0,i]); } }
            }
            else
            {
                for (int i = 0; i < countOfColors + 1; i++) { listRangesSourveValue.Add(rangeSum); rangeSum += range; }
            }
            #region считаем количество попаданий в проценты
              countHits = new int[listRangesSourveValue.Count];
                for (int i = 0; i < dtSource.Rows.Count; i++)
                {
                    decimal selectedActiveSourceValue = cr.AnalysData.returnNullDecimal(dtSource.Rows[i][cr.AnalysData.Var.ActiveColSourceTable], 4);
                    ...
                    bool check = false;
                    for (int p = 1; p < listRangesSourveValue.Count; p++)
                    {

                        if (selectedActiveSourceValue > listRangesSourveValue[p - 1] &&
                            selectedActiveSourceValue <= listRangesSourveValue[p])
                        {
                            countHits[p]++;
                            check = true;
                        }
            ...
            #endregion считаем количество попаданий в проценты
            #region таблица пользователя
            //заполняем таблицу пользователя
                for (int i = 0; i < dtSource.Rows.Count; i++)
                {
                    decimal selectedActiveSourceValue = cr.AnalysData.returnNullDecimal(dtSource.Rows[i][cr.AnalysData.Var.ActiveColSourceTable], 5);
                    decimal selectedActiveSourceValueDouble = cr.AnalysData.returnNullDecimal(selectedActiveSourceValue);
                    string selectedActiveSourceValueString = rfNum(selectedActiveSourceValueDouble);

                    //Присваиваю цвета для субъектов                
                    for (int j = 0; j < listRangesSourveValue.Count - 1; j++)
                    {
                        if (Math.Round(selectedActiveSourceValue,2) >= listRangesSourveValue[j] &&
                            Math.Round(selectedActiveSourceValue,2) <= listRangesSourveValue[j + 1])
                        {
                            fillMapSubjects(dtSource.Rows[i][cr.ClNames.Subject_latin_name.nameSQL].ToString(), j, false, Color.White,
                                dtSource.Rows[i][cr.ClNames.Subject_name.nameSQL].ToString(), cr.AnalysData.Var.ActiveColSourceTableText,
                                selectedActiveSourceValueString, cr.AnalysData.returnNullInt(dtSource.Rows[i][cr.ClNames.Code_subject.nameSQL]));
                        }
            ...
            #endregion таблица пользователя
            //добавляем расшифровку
            //
                if (matches.Length / 2 == listRangesSourveValue.Count)
                {
                    for (int i = 0; i < matches.Length / 2; i++)
                    {
                        if (dataIsPercent)
                        {
                            matches[0, i] = cr.AnalysData.returnNullDecimal(listRangesSourveValue[i] * 100, 0);
                        }
                        else
                        {
                            matches[0, i] = cr.AnalysData.returnNullDecimal(listRangesSourveValue[i], 2);
                        }

                    }
                }
            //
            object itemWpMap = mapControl.FindName("subjects_discriptions");
            if (itemWpMap != null)
            {
                System.Windows.Controls.WrapPanel wpMap = (System.Windows.Controls.WrapPanel)itemWpMap;
                foreach (DataRow row in dtSource.Rows)
                {
                    string inBrackets = "";
                    if (sizeValueScale.Length <= 1)
                    {
                        inBrackets = " (" + cr.AnalysData.returnNullDecimal(row[cr.AnalysData.Var.ActiveColSourceTable], 2).ToString() + ")";
                    }
                    System.Windows.Controls.Label lblSubjectDiscription = new System.Windows.Controls.Label();
                    System.Windows.Thickness m=new System.Windows.Thickness();
                    m.Top = 0; m.Bottom = 0;
                    lblSubjectDiscription.Margin = m;
                    m.Top = 1; m.Bottom = 1;
                    lblSubjectDiscription.Padding = m;
                    lblSubjectDiscription.Content = row[cr.ClNames.Code_subject.nameSQL] + " - " + row[cr.ClNames.Subject_name.nameSQL].ToString() + inBrackets + ", ";
                    lblSubjectDiscription.FontSize = 8;
                    wpMap.Children.Add(lblSubjectDiscription);
                }
                itemWpMap = wpMap;
            }
            //
            ...
            //
            MapHeatScale();
            //Прячем ненужные объекты
            if (cr.AnalysData.Var.IdDistrict > 0)
            {
        ...
        private void fillMapSubjects(string name, int index, bool specColor, System.Drawing.Color specialColor, string subject_name, string field, string value, int code=0)
        {
            if (specColor)
            {
            ...
            else
            {
                ...
                item = mapControl.FindName(name + "_text");
                if (item != null)
                {
                    System.Windows.Controls.TextBlock tb = (System.Windows.Controls.TextBlock)item;
                    tb.Text = code.ToString();
                    item = tb;
                }
            }
        ...
        private void MapHeatScale()
        {
            ...
            int countScale = matches.Length/2 - 1;
            //Шкала
            for (int i = 0; i < countScale; i++)
            {
                object item = mapControl.FindName("cvsScale" + ((int)i + (int)1).ToString());
                ApplyMapHeatScale(item, i);
                //stepSum -= step;
            }
            //Дубль шкалы
            //stepSum = countOfColors;
            for (int i = 0; i < countScale; i++)
            {
                object item = mapControl.FindName("cvsScale" + ((int)i + (int)1 + "_2").ToString());
                ApplyMapHeatScale(item, i);
                //stepSum -= step;
            }
            string format = "{0:f2}";
            ...
            if (dataIsPercent)
            {
                minSourceValue = minSourceValue * 100;
                maxSourceValue = maxSourceValue * 100;
            }
            ...
            if (countHits !=null)
            {
                for (int i = 1; i < countHits.Length; i++)
                {
                    itemLable = mapControl.FindName("lblScale" + (i).ToString() + "_2");
                    ApplyMapHeatLables(itemLable, matches[0, i - 1] + " - " + matches[0, i] + " " + sizeValueScale + " (" + countString(countHits[i]) + ")");
                    //ApplyMapHeatLables(itemLable, "0-" + percents[i] + " " + sizeValueScale + " (" + countString(countPercents[i]) + ")");
                }
            }
        ...
        private string countString(int count)
        {
            ...
            {
                if (returnLastInt(count) < 2 && returnLastInt(count) > 0)
                {
                    str = count + " регион";
                }
                else if (returnLastInt(count) > 1 && returnLastInt(count) < 5)
                {
                    str = count + " региона";
                }
                else if (returnLastInt(count) <= 0)
                {
                    str = count + " регионов";
                }
                else
        ...
        private int returnLastInt(int number)
        {
            List<string> arr = new List<string>();
            for (int i = 0; i < number.ToString().Length; i++)
            {
                arr.Add(number.ToString()[i].ToString());
            }
            return cr.AnalysData.returnNullInt(arr[arr.Count - 1]);
        }
        ...
        private void defineTypeDataFormat(string col)
        {
        ...
        private decimal MapHeatLableValue(decimal value)
        {
            ...
            else if(Convert.ToInt32(value / 1000) <= 0)
            {
                sizeValueScale = "";
            }
            else
        ...
        {
            this.buttonScreenshot.Hide();
            //Thread.Sleep(1000);
            Classes.ScreenCapture sc = new Classes.ScreenCapture();
            string text = RemoveSpecialCharacters(this.Text);
            string fullpath = "C:\\" + text + ".png";
            
            sc.CaptureWindowToFile(this.Handle, fullpath, System.Drawing.Imaging.ImageFormat.Png);
            //Thread.Sleep(1000);
            this.buttonScreenshot.Show();
        }
        private static string RemoveSpecialCharacters(string str)
        {
            return Regex.Replace(str, "[^a-zA-Zа-яА-Я0-9_. ]+", "", RegexOptions.Compiled);
        }
