...
namespace NalogUser.Controls
{
    public partial class UserControlAnalysisDataDefault : UserControl
    {
        ...
        {
            if (!this.checkBoxTax.Checked) { this.comboBoxTaxes.Enabled = false; cr.AnalysData.Var.IdTax = 0; cr.AnalysData.Var.IdsTaxes = null; }
            this.comboBoxTaxes.Enabled = this.checkBoxTax.Checked;
            this.buttonSelectTaxes.Enabled = this.checkBoxTax.Checked;
        }
        ...
        private void UserControlAnalysisDataDefault_Load(object sender, EventArgs e)
        {
            ...
            //
            this.taxes_budgetsTableAdapter.Fill(this.base_nalogDataSet.Taxes_budgets);
            this.scheme_valuesTableAdapter.Fill(this.base_nalogDataSet.Scheme_values);
        }
        ...
        {
            if (this.checkBoxBudget.Checked) { this.comboBoxBudgets.Enabled = true; } else { this.comboBoxBudgets.Enabled = false; cr.AnalysData.Var.IdBudget = 0; cr.AnalysData.Var.IsBudget = false; }
        }

        private void buttonSelectTaxes_Click(object sender, EventArgs e)
        {
            OpenFormForSelectParams(this.taxesBindingSource, this.comboBoxTaxes.DisplayMember, this.comboBoxTaxes.ValueMember, "t");
        }

        private void buttonIndexes_Click(object sender, EventArgs e)
        {
            OpenFormForSelectParams(this.schemevaluesBindingSource, this.comboBoxSchemeValues.DisplayMember, this.comboBoxSchemeValues.ValueMember, "i");
        }
