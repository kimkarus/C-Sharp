...
namespace NalogAdministrator.Classes.ImpData.Methods
{
    public class ImportDataReport
    {
        ...
        //
            ...
            private void eventOnChoiceReport()
            {
                var.TableAdapterMethod = returnTableAdapterMethod(cr.ImportData.ImpDataFile.Type);
            ...
            private string returnTableAdapterMethod(string str)
            {
                if (str.Length <= 0) str = "";
                //
                if(str.Contains("1NM")) str = "1NM";
                if (str.Contains("4NM")) str = "4NM";
                if (str.Contains("1NOM")) str = "1NOM";
                if (str.Contains("1PATENT")) str = "1PATENT";
                if (str.Contains("5ENVD")) str = "5ENVD";
                if (str.Contains("5USN")) str = "5USN";
                //
                return str;
            }
            private void AcceptMethodImport(int j, bool importOneList = false)
            {
                    ...
                    var.ChAllInOne)
                {
                    ...
                    if(var.InternalTable != null) var.InternalTable.Clear();
                    if (isListOk)
                    {
    ...
    var.Ch5USN && var.ChAllInOne)
            ...
            private void AddIntervalTableData1NM_AllInOne(int year)
            {
                ...
                do
                {
                    ...
                    for (int r = 0; r < 30; r++)
                    {
                        ...
                        if (strIndex != "")
                        {
                            if (strIndex == "A" || strIndex == "А") { rowIndexCodeTax = r; break; }
                        }
                    }
                    chechAllRows = true;
                ...
                for (int c = 1; c < var.InternalTable.Columns.Count; c++)
                {
                    ...
                    string strTaxColumnName = var.InternalTable.Rows[rowIndexCodeTax][c].ToString();
                    string[] arrCodeTax = strTaxColumnName.Split('.');
                    if (arrCodeTax[0].Length <= 0) continue;
                            ...
                            for (int j = 0; j < var.ListTaxesColsIds.Count; j++)
                            {
                                ...
                                if (codeTax > 0 && subj > 0 && year > 0)
                                {
                                    ...
                                    if (impData.returnNullBool(arrCodeTax[0, 3]) && existNeedTax1NM(codeTax))
                                    {
                                        ...
                                        count2++;
                                        int col_index = impData.returnNullInt(arrCodeTax[0, 2]);
                                        //bool existColumn = checkExistColumnTableByName(var.InternalTable, prefixColumn);

                                        if (var.InternalTable.Rows[i][col_index] != System.DBNull.Value && checkedImprotedData != 3)
                                        {
                                            ...
                                            if (isColumnSourceTable(arrCodeTax[0, 1]))
                                            {
                                                ...
                                                if (checkedImprotedData == 0 || checkedImprotedData == 1)
                                                {
                                                    ...
                                                    row[arrCodeTax[0, 1]] = impData.returnNullInt(var.InternalTable.Rows[i][col_index]);
                                                ...
                                                if (checkedImprotedData == 0)
                                                {
                                                    if(row.RowState == DataRowState.Modified) row.SetAdded();
            ...
            private bool checkExistColumnTableByName(DataTable dt, string name)
            {
                bool yes = false;
                foreach (DataColumn cl in dt.Columns)
                {
                    if (cl.ColumnName == name) yes = true;
                }
                return yes;
            }
