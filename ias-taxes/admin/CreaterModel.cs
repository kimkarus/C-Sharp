...
namespace NalogAdministrator.Classes
{
    public class CreaterModel
    {
        ...
        private DataTable returnMultiDinamicsDt(DataTable Dt0, int startIndexRow = 1, int startIndexCol = 1, bool deleteFirstRow = true)
        {
            DataTable Dt1 = Dt0.Copy();
            DataTable Dt2 = Dt0.Copy();
            Dt1.TableName = Dt1.TableName + "_1";
            ...
            if(deleteFirstRow) Dt2.Rows[0].Delete();
            Dt2.AcceptChanges();
            //
            return Dt2;
        }
        ...
        {
            if (cr.ImportData.returnNullDouble(prev) == 0.00 || prev == null) return 0;
            decimal res = curr / prev;
            return res;
        }
        ...
        {
            DataTable dts1, dts2 = null;
            #region отбираем субъекты
            if (idSubjects == null)
            {
                idSubjects = cr.ImportData.ReturnSourceTable("SELECT     id as id_subject  FROM         dbo.Subjects", "idSubjects");
            }
            id_subject = cr.ImportData.Var.IdSubject;
            #endregion отбираем субъекты
            #region устанавливаем параметр лет
            //
            //years
            ...
            string query1 =
                ...
                "FROM      dbo.Data_warehouse_subject_tax_budget INNER JOIN " +
                           "dbo.Time ON dbo.Data_warehouse_subject_tax_budget.id_time = dbo.Time.id " +
                "WHERE     (dbo.Data_warehouse_subject_tax_budget.id_subject = " + id_subject + ") " +
                            "AND (dbo.Data_warehouse_subject_tax_budget.id_tax = " + cr.ImportData.Var.IdTax + ") " +
                            "AND (dbo.Data_warehouse_subject_tax_budget.id_budget = " + cr.ImportData.Var.CodeBudget + ") " + whereYears +
                "ORDER BY calendar_year";

            dts1 = cr.ImportData.ReturnSourceTable(query1, "Order1");
            string query2 = 
            ...
            if (id_subject > 0)
            {
                dts2 = cr.ImportData.ReturnSourceTable(query2, "Order2");
                dts2 = returnMultiDinamicsDt(dts2, 1, 0, false);
            }
            if (dts1.Rows.Count <= 0 || dts2.Rows.Count <= 0) return;
            //
            //RDot
            ...
            engine.Initialize();
            //
            transerDataToR(dts1,dts2);
            //
            engine.Evaluate("save.image(file='f.RData')");
            //engine.Dispose();
        }
        private void transerDataToR(DataTable Dt1, DataTable Dt2, int startColFactorIndex = 0)
        {
            //libraries
            engine.Evaluate("library('R.matlab')");
            engine.Evaluate("library('deepnet')");
            engine.Evaluate("library('caret')");
            engine.Evaluate("library('h2o')");
            engine.Evaluate("library('TTR')");
            engine.Evaluate("library('rminer')");
            engine.Evaluate("library('foreach')");
            engine.Evaluate("library('doParallel')");
            engine.Evaluate("library('svSocket')");
            engine.Evaluate("library('lattice')");
            engine.Evaluate("library('ggplot2')");
            engine.Evaluate("library('statmod')");
            engine.Evaluate("library('kknn')");
            engine.Evaluate("library('iterators')");
            engine.Evaluate("library('parallel')");
            engine.Evaluate("library('quantmod')");
            //variables
            var ch1 = engine.CreateInteger(5);
            var ch2 = engine.CreateInteger(20);
            engine.SetSymbol("ch1", ch1);
            engine.SetSymbol("ch2", ch2);
            var startYear = engine.CreateInteger(cr.ImportData.Var.YearAfter);
            engine.SetSymbol("startYear", startYear);
            //result
            #region set result
            double[,] DataResult = new double[Dt1.Rows.Count, Dt1.Columns.Count];
            for (int row = 0; row < Dt1.Rows.Count; row++)
            {
                for (int col = 0; col < Dt1.Columns.Count; col++)
                {
                    DataResult[row, col] = cr.ImportData.returnNullDouble(Dt1.Rows[row][col]);
                }
            }
            NumericMatrix matrixResult = engine.CreateNumericMatrix(DataResult);
            engine.SetSymbol("resultIN", matrixResult);
            #endregion set result
            //factors
            #region factors
            int countCol = Dt2.Columns.Count;
            List<string> idsCols = new List<string>();
            #region clean factors
            for (int col = 0; col < countCol; col++)
            {
                Dt2.Rows[0][col] = 0;
            }
            Dt2.AcceptChanges();
            for (int col = startColFactorIndex; col < countCol; col++)
            {
                double sum = 0;
                for (int row = 0; row < Dt2.Rows.Count; row++)
                {
                    double src = cr.ImportData.returnNullDouble(Dt2.Rows[row][col]);
                    if (src == 0) sum++;
                    else if (src == 1) sum++;
                    else { }
                }
                double diff = 0;
                if (sum > 0 && Dt2.Rows.Count > 0) diff = sum / Dt2.Rows.Count;
                if (diff > 0.85) idsCols.Add(Dt2.Columns[col].ColumnName.ToString());
                
            }
            for (int i = 0; i < idsCols.Count; i++)
            {
                Dt2.Columns.Remove(idsCols[i]);
            }
            for (int i = 0; i < startColFactorIndex; i++)
            {
                string colName = Dt2.Columns[i].ColumnName.ToString();
                Dt2.Columns.Remove(colName);
            }
            Dt2.AcceptChanges();
            #endregion clean factors
            #region set factors
            countCol = Dt2.Columns.Count;
            double[,] DataFactors = new double[Dt2.Rows.Count, countCol];
            for (int row = 0; row < Dt2.Rows.Count; row++)
            {
                for (int col = 0; col < countCol; col++)
                {
                    if (row == 0)
                    {
                        DataFactors[row, col] = 0;
                    }
                    else
                    {
                        DataFactors[row, col] = cr.ImportData.returnNullDouble(Dt2.Rows[row][col]);
                    }
                }
            }
            NumericMatrix matrixFactors = engine.CreateNumericMatrix(DataFactors);
            engine.SetSymbol("factorsIN", matrixFactors);
            //names factors
            string[] factorsNames = new string[Dt2.Columns.Count];
            for (int j = 0; j < Dt2.Columns.Count; j++)
            {
                factorsNames[j] = Dt2.Columns[j].ColumnName.ToString();
            }
            CharacterVector vectorFactorsNames = engine.CreateCharacterVector(factorsNames);
            engine.SetSymbol("factorsNames", vectorFactorsNames);
            #endregion set factors
            #endregion factors
            //start script futnctions
            engine.Evaluate("source('i_SAE_fun.r')");
            //methods
            engine.Evaluate("zz1 <- ZZ(ch1, pr = resultIN)");
            engine.Evaluate("zz2 <- ZZ(ch2, pr = resultIN)");
            engine.Evaluate("Y <- Out(zz1, zz2)");
            //engine.Evaluate("factorsTS <- ts(factorsIN, start = startYear)");
            //engine.Evaluate("factorsTS <- factorsTS/lag(factorsTS,-1) - 1");
            //engine.Evaluate("factorsTS <- Delt(factors)");
            //engine.Evaluate("factorsOUT <- rbind(c(NA),factorsTS)");
            //engine.Evaluate("factorsOUT <- data.matrix(factorsOUT)");
            engine.Evaluate("X <- factorsIN");
            engine.Evaluate("dt <- Clearing(X, Y)");
            
            //engine.Evaluate("source('i_SAE.r')");
        }
