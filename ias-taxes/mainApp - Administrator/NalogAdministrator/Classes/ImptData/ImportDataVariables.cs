...
namespace NalogAdministrator.Classes.ImpData
{
    public class ImportDataVariables
    {
        ...
        public void OpenExcelSheet(string sheet, int l=-1)
        {
            //try
            //{
            ...
            try
            {
                ...
                {
                    int index = i + 1;
                    Excel.Worksheet testSheet = (Excel.Worksheet)xlWorkBook.Worksheets[index];
                    if (sheet == testSheet.Name) l = index;
                }
                if (l > 0)
                {
                    xlWorkSheet = (Excel.Worksheet)xlWorkBook.Worksheets[l];
                }
            }
