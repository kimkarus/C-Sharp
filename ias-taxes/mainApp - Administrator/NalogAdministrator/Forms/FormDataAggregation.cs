...
namespace NalogAdministrator.Forms
{
    public partial class FormDataAggregation : Form
    {
        ...
        private void checkBoxLineSubjects_CheckedChanged(object sender, EventArgs e)
        {
            //cr.ImportData.Var.AddLineSubjects = checkBoxLineSubjects.Checked;
        }
    }
    public class Agregation
    {
        ...
        public Agregation(Classes.Core Cr, Forms.FormDataAggregation FrmAgr)
        {
            ...
            cr.Data.Base.DataWarehouseSubjectTaxBudgetsTableAdapter = new base_nalogDataSetTableAdapters.Data_warehouse_subject_tax_budgetTableAdapter();
            cr.Data.Base.DataWarehouseSubjectEeaBudgetsTableAdapter = new base_nalogDataSetTableAdapters.Data_warehouse_subject_eea_budgetTableAdapter();
            cr.Data.Base.DataWarehouseSubjectBudgetsTableAdapter = new base_nalogDataSetTableAdapters.Data_warehouse_subject_budgetTableAdapter();
            cr.Data.Base.DataWarehouseCountryTaxTableAdapter = new base_nalogDataSetTableAdapters.Data_warehouse_country_taxTableAdapter();
            //
        }
        //
        private string[,] arrTypeAgregation = { 
                                              ...
                                              {"Агрегация данных по итоговым показателям Федеральные округа и виды экономической деятельности (ГКС)", "district_eea_gks", "Data_warehouse_district_eea_gks","Federal_district", "id_district"},
                                              {"Агрегация данных по итоговым показателям субъектов по бюджетам","subject_budget", "Data_warehouse_subject_budget", "Subjects", "id_subject"},
                                              {"Агрегация данных по итоговым показателям субъектов, видам налогов и бюжетам", "subject_tax_budget", "Data_warehouse_subject_tax_budget", "Subjects", "id_subject"},
                                              {"Агрегация данных по итоговым показателям субъектов, ВЭД и бюжетам", "subject_eea_budget", "Data_warehouse_subject_eea_budget", "Subjects", "id_subject"},
                                              {"Агрегация данных по итоговым показателям страны в разрезе виднов налогов", "country_tax", "Data_warehouse_country_tax", "Countries", "id_country"}};
        ...
        private FederalDistrictEeaGksAgregation fdEeaGks;
        private SubjectBudgetAgregation sbjBudget;
        private SubjectTaxBudgetAgregation sbjTaxBudget;
        private SubjectEeaBudgetAgregation sbjEeaBudget;
        ...
        public bool IsTypeAgregation(string type)
        {
            ...
            if (type.Length > 0)
            {
                ...
                switch (currentTypeAgregationName)
                {
                    ...
                    case "district":
                        {
                            fd = new FederalDistrictAgregation(this);
                    ...
                    case "district_eea_gks":
                        {
                            fdEeaGks = new FederalDistrictEeaGksAgregation(this);
                            listParamsAgregation = fdEeaGks.ListParamsAgregation;
                            commandsAgregation = fdEeaGks.CommandsAgregation;
                            tableAgregation = fdEeaGks.TableAgregation;
                            isSubject = fdEeaGks.IsSubject;
                            isDistrict = fdEeaGks.IsDistrict;
                            isTax = fdEeaGks.IsTax;
                            isEea = fdEeaGks.IsEea;
                            rowAgr = fdEeaGks.Row;
                            //idSelectMethod = checkSelectMethod(sbjEeaGks.SelectMethod);
                            break;
                        }
                    case "subject_budget":
                        {
                            sbjBudget = new SubjectBudgetAgregation(this);
                            listParamsAgregation = sbjBudget.ListParamsAgregation;
                            commandsAgregation = sbjBudget.CommandsAgregation;
                            tableAgregation = sbjBudget.TableAgregation;
                            isSubject = sbjBudget.IsSubject;
                            isDistrict = sbjBudget.IsDistrict;
                            isTax = sbjBudget.IsTax;
                            isEea = sbjBudget.IsEea;
                            isBudget = sbjBudget.IsBudget;
                            rowAgr = sbjBudget.Row;
                            //idSelectMethod = checkSelectMethod(sbjEeaGks.SelectMethod);
                            break;
                        }
                    case "subject_tax_budget":
                        {
                            sbjTaxBudget = new SubjectTaxBudgetAgregation(this);
                            listParamsAgregation = sbjTaxBudget.ListParamsAgregation;
                            commandsAgregation = sbjTaxBudget.CommandsAgregation;
                            tableAgregation = sbjTaxBudget.TableAgregation;
                            isSubject = sbjTaxBudget.IsSubject;
                            isDistrict = sbjTaxBudget.IsDistrict;
                            isTax = sbjTaxBudget.IsTax;
                            isEea = sbjTaxBudget.IsEea;
                            isBudget = sbjTaxBudget.IsBudget;
                            rowAgr = sbjTaxBudget.Row;
                            //idSelectMethod = checkSelectMethod(sbjEeaGks.SelectMethod);
                            break;
                        }
                    case "subject_eea_budget":
                        {
                            sbjEeaBudget = new SubjectEeaBudgetAgregation(this);
                            listParamsAgregation = sbjEeaBudget.ListParamsAgregation;
                            commandsAgregation = sbjEeaBudget.CommandsAgregation;
                            tableAgregation = sbjEeaBudget.TableAgregation;
                            isSubject = sbjEeaBudget.IsSubject;
                            isDistrict = sbjEeaBudget.IsDistrict;
                            isTax = sbjEeaBudget.IsTax;
                            isEea = sbjEeaBudget.IsEea;
                            isBudget = sbjEeaBudget.IsBudget;
                            rowAgr = sbjEeaBudget.Row;
                            //idSelectMethod = checkSelectMethod(sbjEeaGks.SelectMethod);
                            break;
                        }
        ...
        public void startAgregation(string type = "")
        {
            ...
            //GINI index
            ParamAgregation prmCheck = (ParamAgregation)listParamsAgregation[0];
            string[] arrCheckSourceField = prmCheck.FieldSourceTable.Split('_');
            foreach(string str in arrCheckSourceField)
            {
                if(str.ToUpper() == "GINI")
                {
                    isGini=true;
            ...
            if (!isMergeTable)
            {
                ...
                for (int j = 0; j < listParamsAgregation.Count; j++)
                {
                    ...
                    if (selectWhere.Length > 0 && !isGini)
                    {
                        prm.SqlSelect = prm.SqlSelect + " WHERE " + selectWhere;
                    }
                    else if (selectWhere.Length > 0 && isGini)
                    {
                        prm.SqlSelect = prm.SqlSelect;
                    }
                    else
                    {
                    }
                    
                    if (!frmAgr.checkBoxFilterSubject.Checked && frmAgr.checkBoxLineSubjects.Checked)
                    {
                        foreach (DataRow row in ds.Subjects.Rows)
                        {
                            string select = prm.SqlSelect;
                            if (selectWhere.Length > 0)
                            {
                                select += " AND id_subject=" + row["id"];
                            }
                            else
                            {
                                select += " WHERE id_subject=" + row["id"];
                            }
                    ...
                    else if (frmAgr.checkBoxLineDistricts.Checked)
                    {
                        if (cr.Data.Base.FederalDistrictsTableAdapter == null)
                        {
                            cr.Data.Base.FederalDistrictsTableAdapter = new base_nalogDataSetTableAdapters.Federal_districtTableAdapter();
                        }
                        if (ds.Federal_district == null)
                        {
                            cr.Data.Base.FederalDistrictsTableAdapter.Fill(ds.Federal_district);
                        }
                        foreach (DataRow row in ds.Federal_district.Rows)
                        {
                            string selectDistrict = prm.SqlSelect;
                            string selectYear = "";
                            string select = "";
                            
                            if (isGini)
                            {
                                selectDistrict += " WHERE id_district=" + row["id"];
                                foreach (int year in var.YearsList)
                                {
                                    selectYear = "";
                                    if (selectDistrict.Length > 0)
                                    {
                                        selectYear += " AND year=" + year;
                                    }
                                    else
                                    {
                                        selectYear += " WHERE year=" + year;
                                    }
                                    select = selectDistrict + selectYear + " ORDER BY accum_share_ti";
                                    cr.ImportData.Var.InternalTable = cr.ImportData.ReturnSourceTable(select, prm.TableName);
                                    processingInternalTable(prm, j);
                                    cr.ImportData.Var.InternalTable.Clear();
                                }
                            ...
                            {
                                select = prm.SqlSelect;
                                if (selectWhere.Length > 0)
                                {
                                    select += " AND id_district=" + row["id"];
                                }
                                else
                                {
                                    select += " WHERE id_district=" + row["id"];
                                }
                                cr.ImportData.Var.InternalTable = cr.ImportData.ReturnSourceTable(select, prm.TableName);
                                processingInternalTable(prm, j);
                                cr.ImportData.Var.InternalTable.Clear();
        ...
        private void processingInternalTable(ParamAgregation prm, int stepIndex = 0, int stepSelectIndex = 0, int countStepsSelectIndex = 0, bool isMergeTable = false, string subject_name="")
        {
            ...
            switch (currentTypeAgregationName)
            {
                ...
                case "district_eea_gks":
                    {
                        if (cr.Data.Base.DataWarehouseDistrictEeaGksTableAdapter == null) cr.Data.Base.DataWarehouseDistrictEeaGksTableAdapter = new base_nalogDataSetTableAdapters.Data_warehouse_district_eea_gksTableAdapter();
                        cr.Data.Base.DataWarehouseDistrictEeaGksTableAdapter.Fill(ds.Data_warehouse_district_eea_gks);
                        break;
                    }
                case "subject_budget":
                    {
            ...
            try
            {
                decimal testValue = cr.ImportData.returnNullDecimal(cr.ImportData.Var.InternalTable.Rows[0][prm.FieldSourceTable], prm.Round);
            }
            ...
            if (!isGini && !error)
            {
                ...
                for (int i = 0; i < cr.ImportData.Var.InternalTable.Rows.Count; i++)
                {
                    ...
                    if (isSubject) { strKey = "id_subject"; }
                    if (isDistrict) { strKey = "id_district"; }
                    if (isTax && (!isSubject && !isDistrict)) { strKey = "id_tax"; }
                    if (isEea && (!isSubject && !isDistrict)) { strKey = "id_eea"; }
                    idKey = cr.ImportData.returnNullInt(cr.ImportData.Var.InternalTable.Rows[i][strKey]);
                    #endregion Выбор ключевого поля
                    if (isTax || isEea) sqlExt1_Value = cr.ImportData.returnNullInt(cr.ImportData.Var.InternalTable.Rows[i][sqlExt1]);
                    if (isBudget) sqlExt2_Value = cr.ImportData.returnNullInt(cr.ImportData.Var.InternalTable.Rows[i][sqlExt2]);
                    ...
                    if (idKey > 0 || year > 0)
                    {
                        ...
                        if (sqlExt1.Length > 0 && (isEea) && !isBudget)
                        {
                            cr.ImportData.Var.AgregationTable = cr.ImportData.ReturnSourceTable("SELECT " + tableAgregation + ".*, Time.* FROM " + tableAgregation + " INNER JOIN Time ON dbo." + tableAgregation + ".id_time = dbo.Time.id WHERE (dbo." + tableAgregation + "." + strKey + " = " + idKey + ") AND (dbo.Time.calendar_year = " + year + ") AND (dbo." + tableAgregation + "." + sqlExt1 + " = " + sqlExt1_Value + ")", tableAgregation);
                        }
                        if (sqlExt1.Length > 0 && (isTax) && !isBudget)
                        {
                            cr.ImportData.Var.AgregationTable = cr.ImportData.ReturnSourceTable("SELECT " + tableAgregation + ".*, Time.* FROM " + tableAgregation + " INNER JOIN Time ON dbo." + tableAgregation + ".id_time = dbo.Time.id WHERE (dbo." + tableAgregation + "." + strKey + " = " + idKey + ") AND (dbo.Time.calendar_year = " + year + ") AND (dbo." + tableAgregation + "." + sqlExt1 + " = " + sqlExt1_Value + ")", tableAgregation);
                        }
                        else if (sqlExt1.Length > 0 && (isTax || isEea) && (isBudget))
                        {
                            cr.ImportData.Var.AgregationTable = cr.ImportData.ReturnSourceTable("SELECT " + tableAgregation + ".*, Time.* FROM " + tableAgregation + " INNER JOIN Time ON dbo." + tableAgregation + ".id_time = dbo.Time.id WHERE (dbo." + tableAgregation + "." + strKey + " = " + idKey + ") AND (dbo.Time.calendar_year = " + year + ") AND (dbo." + tableAgregation + "." + sqlExt1 + " = " + sqlExt1_Value + ") AND (dbo." + tableAgregation + "." + sqlExt2 + " = " + sqlExt2_Value + ")", tableAgregation);
                        }
                        else if (sqlExt2.Length > 0 && (!isTax && !isEea) && (isBudget))
                        {
                            cr.ImportData.Var.AgregationTable = cr.ImportData.ReturnSourceTable("SELECT " + tableAgregation + ".*, Time.* FROM " + tableAgregation + " INNER JOIN Time ON dbo." + tableAgregation + ".id_time = dbo.Time.id WHERE (dbo." + tableAgregation + "." + strKey + " = " + idKey + ") AND (dbo.Time.calendar_year = " + year + ") AND (dbo." + tableAgregation + "." + sqlExt2 + " = " + sqlExt2_Value + ")", tableAgregation);
                        }
                        else if ((isSubject || isDistrict) && !isBudget)
                        {
                        ...
                        else if ((sqlExt1.Length > 0 && !isSubject && !isDistrict) && (isTax || isEea) && !isBudget)
                        {
                            cr.ImportData.Var.AgregationTable = cr.ImportData.ReturnSourceTable("SELECT " + tableAgregation + ".*, Time.* FROM " + tableAgregation + " INNER JOIN Time ON dbo." + tableAgregation + ".id_time = dbo.Time.id WHERE (dbo." + tableAgregation + "." + sqlExt1 + " = " + sqlExt1_Value + ") AND (dbo.Time.calendar_year = " + year + ")", tableAgregation);
                        }
                        else
                        {

                        }
                        #region Проверка позиции на наличие
                        int countFound = 0;
                        int cur_id = 0;
                        if (cr.ImportData.Var.AgregationTable.Rows.Count > 0)
                        {
                            cur_id = cr.ImportData.returnNullInt(cr.ImportData.Var.AgregationTable.Rows[0]["id"]);
                        }
                        int id = 0;
                        for (int j = 0; j < cr.ImportData.Var.AgregationTable.Rows.Count; j++)
                        {
                            cur_id = cr.ImportData.returnNullInt(cr.ImportData.Var.AgregationTable.Rows[j]["id"]);
                            rowAgr = ds.Tables[arrTypeAgregation[currentTypeAgregationIndex, 2]].Rows.Find(cur_id);
                            
                            if ((isEea || isTax) && !isBudget)
                            {
                                if (rowAgr[sqlExt1].ToString() == sqlExt1_Value.ToString())
                                {
                                    if (cr.ImportData.returnNullDecimal(rowAgr[prm.FieldDestinationTable]) != value)
                                    {
                                        countFound++;
                                        id = cur_id;
                                    }
                            ...
                            else if ((isEea || isTax) && isBudget)
                            {
                                if (rowAgr[sqlExt1].ToString() == sqlExt1_Value.ToString()
                                    && rowAgr[sqlExt2].ToString() == sqlExt2_Value.ToString())
                                {
                                    if (cr.ImportData.returnNullDecimal(rowAgr[prm.FieldDestinationTable]) != value)
                                    {
                                        countFound++;
                            ...
                            {
                                if (rowAgr[sqlExt2].ToString() == sqlExt2_Value.ToString())
                                {
                                    if (cr.ImportData.returnNullDecimal(rowAgr[prm.FieldDestinationTable]) != value)
                                    {
                                        countFound++;
                        ...
                        #endregion Проверка позиции на наличие
                            ...
                            && id > 0)
                        {
                            ...
                            if ((isEea || isTax) && !isBudget)
                            {
                                ...
                                rowAgr[sqlExt1] = sqlExt1_Value;
                                rowAgr[prm.FieldDestinationTable] = value;
                            }
                            else if ((isEea || isTax) && isBudget)
                            {
                                rowAgr[sqlExt1] = sqlExt1_Value;
                                rowAgr[sqlExt2] = sqlExt2_Value;
                                rowAgr[prm.FieldDestinationTable] = value;
                            }
                            else if ((!isEea && !isTax) && isBudget)
                            {
                        ...
                        if(cr.ImportData.Var.AgregationTable.Rows.Count <= 0)
                        {
                            ...
                            //
                            if ((isEea || isTax) && !isSubject && !isDistrict && !isBudget)
                            {
                                rowAgr[sqlExt1] = sqlExt1_Value;
                            }
                            else if ((isEea || isTax) && (isSubject || isDistrict) && !isBudget)
                            {
                                rowAgr[sqlExt1] = sqlExt1_Value;
                                rowAgr[strKey] = idKey;
                            }
                            ...
                            #region поиск и добавление записи в таблицу Time
                            //
                            DataRow[] dRowLev1;
                        ...
                        if (cr.ImportData.Var.AgregationTable.Rows.Count > 1)
                        {
                            ds.Tables[arrTypeAgregation[currentTypeAgregationIndex, 2]].Rows.Find(cr.ImportData.Var.AgregationTable.Rows[1]["id"]).Delete();
                            //rowAgr.Delete();
                            //DataRowState rs = rowAgr.RowState;
                            //ds.Tables[arrTypeAgregation[currentTypeAgregationIndex, 2]].Rows.Remove(rowAgr);
                            
                        }
                    }
                    frmAgr.toolStripProgressBarAgregation.Value = i + 1;
                }
                #endregion Если Не Джинни
            }
            else if(isGini && !error)
            {
                #region Если Джинни
                if (cr.ImportData.Var.InternalTable.Rows.Count > 0)
                {
                    x = new double[cr.ImportData.Var.InternalTable.Rows.Count];
                    y = new double[cr.ImportData.Var.InternalTable.Rows.Count];

                    for (int r = 0; r < y.Length; r++)
                    {
                        x[r] = cr.ImportData.returnNullDouble(cr.ImportData.Var.InternalTable.Rows[r]["accum_share_busyed"]);
                        y[r] = cr.ImportData.returnNullDouble(cr.ImportData.Var.InternalTable.Rows[r]["accum_share_ti"]);
                    }
                    int info = 0;
                    ...
                    alglib.polynomialfit(x, y, 6, out info, out p1, out pr1);
                    ...
                    double sumK105 = 0.5 - sumK1;
                    double gini_index = sumK105 / 0.5;
                    //
                    //
                    //
                    var.StatusApp = " Проверяем ";
                    ...
                    if (isTax && (!isSubject && !isDistrict)) { strKey = "id_tax"; }
                    if (isEea && (!isSubject && !isDistrict)) { strKey = "id_eea"; }
                    idKey = cr.ImportData.returnNullInt(cr.ImportData.Var.InternalTable.Rows[0][strKey]);
                    #endregion Выбор ключевого поля
                    if (isTax || isEea) sqlExt1_Value = cr.ImportData.returnNullInt(cr.ImportData.Var.InternalTable.Rows[0][sqlExt1]);
                    if (isBudget) sqlExt2_Value = cr.ImportData.returnNullInt(cr.ImportData.Var.InternalTable.Rows[0][sqlExt2]);
                    year = cr.ImportData.returnNullInt(cr.ImportData.Var.InternalTable.Rows[0]["year"]);
                    if (isMergeTable)
                    {
                        vl = new ValuesAgregation();
                        for (int j = 0; j < listParamsAgregation.Count; j++)
                        {
                            ...
                            vl.ColValues.Add(prm.FieldSourceTable);
                            vl.ColDestValues.Add(prm.FieldDestinationTable);
                            vl.Values.Add(cr.ImportData.returnNullDecimal(cr.ImportData.Var.InternalTable.Rows[0][prm.FieldSourceTable]));
                        }
                    }
                    else
                    {
                        value = (decimal)gini_index;
                    }
                    id_time = 0;
                    if (idKey > 0 || year > 0)
                    {
                        ...
                        if (sqlExt1.Length > 0 && (isTax || isEea) && !isBudget)
                        {
                            cr.ImportData.Var.AgregationTable = cr.ImportData.ReturnSourceTable("SELECT " + tableAgregation + ".*, Time.* FROM " + tableAgregation + " INNER JOIN Time ON dbo." + tableAgregation + ".id_time = dbo.Time.id WHERE (dbo." + tableAgregation + "." + strKey + " = " + idKey + ") AND (dbo.Time.calendar_year = " + year + ") AND (dbo." + tableAgregation + "." + sqlExt1 + " = " + sqlExt1_Value + ")", tableAgregation);
                        }
                        else if (sqlExt1.Length > 0 && (isTax || isEea) && (isBudget))
                    ...
                    frmAgr.toolStripProgressBarAgregation.Value = 0 + 1;
                }
                #endregion Если Джинни
            }
            var.StatusApp = " Добавляем в БД ";
            doTableAdapter(currentTypeAgregationName, arrTypeAgregation[currentTypeAgregationIndex, 2]);
            count++;
        }
        ...
        private void doTableAdapter(string method, string table="")
        {
            cr.Data.Base.CustomTable = new DataTable();
            if(table.Length > 0)
            {
                cr.Data.Base.CustomTable = ds.Tables[table];
            }
            ...
            switch (method)
            {
                ...
                case "district_eea_gks":
                    {
                        deleteRows = (base_nalogDataSet.Data_warehouse_district_eea_gksDataTable)ds.Data_warehouse_district_eea_gks.GetChanges(DataRowState.Deleted);
                        newRows = (base_nalogDataSet.Data_warehouse_district_eea_gksDataTable)ds.Data_warehouse_district_eea_gks.GetChanges(DataRowState.Added);
                        modifiedRows = (base_nalogDataSet.Data_warehouse_district_eea_gksDataTable)ds.Data_warehouse_district_eea_gks.GetChanges(DataRowState.Modified);
                        break;
                    }
            }
            #endregion method
            ...
            try
            {
                ...
                if (deleteRows != null)
                {
                    switch (method)
                    {
                        ...
                        case "district_eea_gks":
                            {
                                cr.Data.Base.DataWarehouseDistrictEeaGksTableAdapter.Update((base_nalogDataSet.Data_warehouse_district_eea_gksDataTable)deleteRows);
                                break;
                            }
                        case "subject_budget":
                            {
                ...
                if (newRows != null)
                {
                    switch (method)
                    {
                        ...
                        case "subject_tax_budget":
                            {
                                cr.Data.Base.DataWarehouseSubjectTaxBudgetsTableAdapter.Update((base_nalogDataSet.Data_warehouse_subject_tax_budgetDataTable)newRows);
                                break;
                            }
                        case "subject_budget":
                            {
                                cr.Data.Base.DataWarehouseSubjectBudgetsTableAdapter.Update((base_nalogDataSet.Data_warehouse_subject_budgetDataTable)newRows);
                                break;
                            }
                        case "subject_eea_budget":
                            {
                                cr.Data.Base.DataWarehouseSubjectEeaBudgetsTableAdapter.Update((base_nalogDataSet.Data_warehouse_subject_eea_budgetDataTable)newRows);
                                break;
                            }
                        case "tax":
                            {
                        ...
                        case "district_eea_gks":
                            {
                                cr.Data.Base.DataWarehouseDistrictEeaGksTableAdapter.Update((base_nalogDataSet.Data_warehouse_district_eea_gksDataTable)newRows);
                                break;
                            }
                    }
                }
                ...
                if (modifiedRows != null)
                {
                    switch (method)
                    {
                        ...
                        case "subject_budget":
                            {
                                cr.Data.Base.DataWarehouseSubjectBudgetsTableAdapter.Update((base_nalogDataSet.Data_warehouse_subject_budgetDataTable)modifiedRows);
                                break;
                            }
                        case "subject_tax_budget":
                            {
                                cr.Data.Base.DataWarehouseSubjectTaxBudgetsTableAdapter.Update((base_nalogDataSet.Data_warehouse_subject_tax_budgetDataTable)modifiedRows);
                                break;
                            }
                        case "subject_eea_budget":
                            {
                                cr.Data.Base.DataWarehouseSubjectEeaBudgetsTableAdapter.Update((base_nalogDataSet.Data_warehouse_subject_eea_budgetDataTable)modifiedRows);
                                break;
                            }
                        case "tax":
                            {
                        ...
                        case "district_eea_gks":
                            {
                                cr.Data.Base.DataWarehouseDistrictEeaGksTableAdapter.Update((base_nalogDataSet.Data_warehouse_district_eea_gksDataTable)modifiedRows);
                                break;
                            }
                    }
                }
            ...
            private void fillListParamAgregation()
            {
                ...
                if (countCommands > 0)
                {
                    foreach (int i in agr.frmAgr.chkListBoxAgregationParam.CheckedIndices)
                    {
                        ...
                        prm.SqlSelect = commandsAgregation[i, 0] + " FROM " + prm.TableName;
        ...
        public class SubjectBudgetAgregation
        {
            Agregation agr;
            private ArrayList listParamsAgregation = new ArrayList();
            private string tableAgregation = "Data_warehouse_subject_budget";
            private string tableSample = "subject_budget";
            private base_nalogDataSet.Data_warehouse_subject_budgetRow row; //subject_budget
            //
            private bool isSubject = true;
            private bool isDistrict = false;
            private bool isTax = false;
            private bool isEea = false;
            private bool isBudget = true;
            private string selectMethod = "one_subject";
            private string[,] commandsAgregation;

            //
            public SubjectBudgetAgregation(Agregation Agr)
            {
                this.agr = Agr;
                commandsAgregation = agr.returnCommandsAgregation(tableSample);
                fillListParamAgregation();
            }
            public SubjectBudgetAgregation()
            {
            ...
            public bool IsTax { get { return isTax; } }
            public bool IsEea { get { return isEea; } }
            public bool IsBudget { get { return isBudget; } }
            public string SelectMethod { get { return selectMethod; } }
            public base_nalogDataSet.Data_warehouse_subject_budgetRow Row { get { return row; } }
            public string[,] CommandsAgregation { get { return commandsAgregation; } }
            //public int IdSelectMethod { get { return idSelectMethod; } set { idSelectMethod = value; } }
            //
            private void fillListParamAgregation()
        ...
        public class SubjectTaxBudgetAgregation
        {
            Agregation agr;
            private ArrayList listParamsAgregation = new ArrayList();
            private string tableAgregation = "Data_warehouse_subject_tax_budget";
            private string tableSample = "subject_tax_budget";
            private base_nalogDataSet.Data_warehouse_subject_tax_budgetRow row; //subject_tax_budget
            //
            private bool isSubject = true;
            private bool isDistrict = false;
            private bool isTax = true;
            ...
            //
            public SubjectTaxBudgetAgregation(Agregation Agr)
            {
                this.agr = Agr;
                commandsAgregation = agr.returnCommandsAgregation(tableSample);
                fillListParamAgregation();
            }
            public SubjectTaxBudgetAgregation()
            {
            ...
            public bool IsBudget { get { return isBudget; } }
            public string SelectMethod { get { return selectMethod; } }
            public base_nalogDataSet.Data_warehouse_subject_tax_budgetRow Row { get { return row; } }
            public string[,] CommandsAgregation { get { return commandsAgregation; } }
            //public int IdSelectMethod { get { return idSelectMethod; } set { idSelectMethod = value; } }
            //
            private void fillListParamAgregation()
        ...
        public class SubjectEeaBudgetAgregation
        {
            Agregation agr;
            private ArrayList listParamsAgregation = new ArrayList();
            private string tableAgregation = "Data_warehouse_subject_eea_budget";
            private string tableSample = "subject_eea_budget";
            private base_nalogDataSet.Data_warehouse_subject_eea_budgetRow row; //subject_tax_budget
            //
            private bool isSubject = true;
            private bool isDistrict = false;
            private bool isTax = false;
            ...
            //
            public SubjectEeaBudgetAgregation(Agregation Agr)
            {
                this.agr = Agr;
                commandsAgregation = agr.returnCommandsAgregation(tableSample);
                fillListParamAgregation();
            }
            public SubjectEeaBudgetAgregation()
            {
            ...
            public bool IsBudget { get { return isBudget; } }
            public string SelectMethod { get { return selectMethod; } }
            public base_nalogDataSet.Data_warehouse_subject_eea_budgetRow Row { get { return row; } }
            public string[,] CommandsAgregation { get { return commandsAgregation; } }
            //public int IdSelectMethod { get { return idSelectMethod; } set { idSelectMethod = value; } }
            //
            private void fillListParamAgregation()
        ...
        public class FederalDistrictAgregation
        {
            ...
            public bool IsTax { get { return isTax; } }
            public bool IsEea { get { return isEea; } }
            public bool IsGini { get { return isGini; } set { isGini = value; } }
            public string SelectMethod { get { return selectMethod; } }
        ...
        public class FederalDistrictEeaGksAgregation
        {
            Agregation agr;
            private ArrayList listParamsAgregation = new ArrayList();
            private string tableAgregation = "Data_warehouse_district_eea_gks";
            private string tableSample = "district_eea_gks";
            private base_nalogDataSet.Data_warehouse_district_eea_gksRow row; //subject
            //
            private bool isSubject = false;
            private bool isDistrict = true;
            private bool isTax = false;
            ...
            private string[,] commandsAgregation;
            //
            public FederalDistrictEeaGksAgregation(Agregation Agr)
            {
                this.agr = Agr;
                commandsAgregation = agr.returnCommandsAgregation(tableSample);
                fillListParamAgregation();
            }
            public FederalDistrictEeaGksAgregation()
            {
            ...
            public bool IsEea { get { return isEea; } }
            public string SelectMethod { get { return selectMethod; } }
            public base_nalogDataSet.Data_warehouse_district_eea_gksRow Row { get { return row; } }
            //public int IdSelectMethod { get { return idSelectMethod; } set { idSelectMethod = value; } }
            //
            private void fillListParamAgregation()
            {
        ...
        public class CountryTaxAgregation
        {
            Agregation agr;
            private ArrayList listParamsAgregation = new ArrayList();
            private string tableAgregation = "Data_warehouse_country_tax";
            private string tableSample = "country_tax";
            private base_nalogDataSet.Data_warehouse_district_eea_gksRow row; //country
            //
            private bool isCountry = true;
            private bool isSubject = false;
            private bool isDistrict = false;
            private bool isTax = true;
            private bool isEea = false;
            ...
            private string[,] commandsAgregation;
            //
            public CountryTaxAgregation(Agregation Agr)
            {
                this.agr = Agr;
                commandsAgregation = agr.returnCommandsAgregation(tableSample);
                fillListParamAgregation();
            }
            public CountryTaxAgregation()
            {
